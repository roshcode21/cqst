...
(() => {
  ...
  $("#shareButton")?.addEventListener("click",async()=>{ const data={title:body.dataset.title||document.title,text:document.querySelector('meta[name="description"]')?.content||"",url:window.location.href}; if(navigator.share){ try{await navigator.share(data);track("article_share",{method:"native"});return;}catch(error){if(error?.name==="AbortError")return;} } try{await navigator.clipboard.writeText(window.location.href);showToast("Enlace copiado");track("article_share",{method:"copy"});}catch{showToast("Copia el enlace desde tu navegador");} });
})();